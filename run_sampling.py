...
# ---------------------------------------------------------------------------
# Amino acid alphabet – must match score matrix row order
# ---------------------------------------------------------------------------
_AA_ORDER = list('ACDEFGHIKLMNPQRSTVWY')
_AA_INDEX  = {aa: i for i, aa in enumerate(_AA_ORDER)}


def _score_peptides_vec(peptides, score_matrix):
    """
    Vectorised CPL scoring: score all peptides against a (20, L) log-prob matrix.

    Replaces the original per-peptide Python loop. Uses NumPy fancy indexing to
    gather log-probabilities for every (peptide, position) pair in one shot,
    then sums across positions and divides by peptide length (length normalisation).

    Parameters
    ----------
    peptides : list[str]
        All peptides must have length == score_matrix.shape[1].
    score_matrix : np.ndarray, shape (20, L)
        Log-probability matrix; rows = amino acids (ACDEFGHIKLMNPQRSTVWY),
        columns = positions 1..L.

    Returns
    -------
    scores : np.ndarray, shape (N,)
        Length-normalised sum of log-probs. Unknown amino acids → NaN.
    """
    if len(peptides) == 0:
        return np.array([], dtype=np.float64)
    pep_len = score_matrix.shape[1]
    n       = len(peptides)
    # Build integer index array (N, L): AA → row index in score_matrix
    aa_idx = np.full((n, pep_len), fill_value=-1, dtype=np.int8)
    for i, pep in enumerate(peptides):
        for j, aa in enumerate(pep[:pep_len]):
            aa_idx[i, j] = _AA_INDEX.get(aa, -1)
    # Rows with any unknown AA receive NaN
    valid_mask = (aa_idx >= 0).all(axis=1)
    col_pos    = np.arange(pep_len, dtype=np.int16)
    scores     = np.full(n, np.nan, dtype=np.float64)
    if valid_mask.any():
        valid_idx  = aa_idx[valid_mask]                    # (N_valid, L)
        log_probs  = score_matrix[valid_idx, col_pos]      # (N_valid, L)
        scores[valid_mask] = log_probs.sum(axis=1) / pep_len  # length-normalise
    return scores
...
class PeptideScoringPipeline:
    def __init__(self, args):
        ...
        self.args          = args
        self.failed_jobs   = []
        assert 'protienmpnn' in os.listdir(self.input_dir), \
            f'{self.input_dir} ::: \n{os.listdir(self.input_dir)}'
        assert 'alphafold' in os.listdir(self.input_dir), \
            f'{self.input_dir} ::: \n{os.listdir(self.input_dir)}'
    ...
    def _read_df(self, df_path=None):
        """Read input CSV/TSV; auto-detect separator."""
        path     = df_path if df_path else self.input_df_path
        df       = pd.read_csv(path)
        required = ['peptide', 'mhc_seq', 'mhc_type', 'anchors', 'id', 'name']
    ...
    def pae_and_plddt(self, id, mhc_seq, pep_seq, name):
        """Compute mean PAE and pLDDT for a given structure."""
        ...
        self.logger.info(
            f"Computed PAE/pLDDT for {name}/{id}: "
            f"PAE_mean={np.mean(pae_peptide):.3f}, "
            f"pLDDT_mean={np.mean(plddt_peptide):.3f}"
        )
        return pae_peptide, plddt_peptide

    def score_matrix(self, id, peptide, name):
        """Compute and save the combined (MPNN + Hermes) score matrix for one id."""
        ...
        for path in os.listdir(protienmpnn_path):
            ...
            try:
                ...
                hermes_df, _ = run_hermes_on_pdbfile_or_pyrosetta_pose(
                    'hermes_py_050', pdb_path,
                    chain_and_sites_list=chain_sites, request=['probas']
                )
                hermes_df_prob = hermes_df.iloc[:, 5:]
                log_hermes_df_prob = np.log(hermes_df_prob)
                log_hermes_df_prob.columns = [
                    i.split('_')[1] for i in log_hermes_df_prob.columns
                ]
                ...
                log_arithmetic_mean = (
                    logsumexp(np.stack([log_p1, log_p2]), axis=0) - np.log(2)
                )
                combined_score = pd.DataFrame(
                    log_arithmetic_mean, index=list('ACDEFGHIKLMNPQRSTVWY')
                )
                combined_score.to_csv(
                    os.path.join(output_path, f'{prefix}combined_score.csv')
                )
                comb_scores.append(combined_score.to_numpy())
                self.logger.info(f"Computed MPNN+Hermes scores for {name}/{id}/{path}")
            except ImportError:
                self.logger.warning(
                    f"Hermes not available, using MPNN score only for {id}/{path}"
                )
        ...
        if len(comb_scores) > 1:
            log_arithmetic_mean = (
                logsumexp(np.stack(comb_scores), axis=0) - np.log(len(comb_scores))
            )
            combined_score = pd.DataFrame(
                log_arithmetic_mean, index=list('ACDEFGHIKLMNPQRSTVWY')
            )
            ...
            combined_score = pd.DataFrame(
                comb_scores[0], index=list('ACDEFGHIKLMNPQRSTVWY')
            )
            combined_score.to_csv(os.path.join(output_path, 'combined_score.csv'))
    ...
    def _parse_chain_and_sites(self, value):
        ...
        for item in value.split():
            ...
            result.append((chain, sites.split(',')))
        return result

    def combine_and_score_peptides(self, df_with_pae_plddt, name):
        """
        Normal mode: average score matrices across all surviving ids under *name*,
        score every unique peptide in the pooled set, record which ids each peptide
        was sampled from (semicolon-separated), and divide scores by peptide length.

        Parameters
        ----------
        df_with_pae_plddt : pd.DataFrame
        name : str
        ...
        if self.args.pae_threshold:
            ...
            self.logger.info(
                f"Filtered by PAE threshold {self.args.pae_threshold}: "
                f"{len(subset_df)} rows remaining"
            )
            ...
            self.logger.info(
                f"Filtered by top {self.args.top_pae_percent * 100}% PAE: "
                f"{len(subset_df)} rows remaining"
            )
        if len(subset_df) == 0:
            raise ValueError(
                f"No valid pae for structures found, try using "
                f"top_pae_percent or adjust pae threshold."
            )
        self.logger.info(f"Minimum pae: {min_pae}")
        # SCORES[key]        = list of (20, L) arrays, one per surviving id
        # PEPTIDE_POOLS[key] = {pep_seq: set(ids)}  – tracks source ids per peptide
        SCORES        = {}
        PEPTIDE_POOLS = {}
        for _, row in subset_df.iterrows():
            ...
            comb_path = os.path.join(
                self.output_dir, name, id_str, 'combined_score.csv'
            )
            if not os.path.exists(comb_path):
                self.logger.warning(
                    f"combined_score.csv not found for {name}/{id_str}"
                )
                continue
            comb_score = pd.read_csv(comb_path, index_col=0).to_numpy()
            SCORES.setdefault(key, []).append(comb_score)
            if key not in PEPTIDE_POOLS:
                PEPTIDE_POOLS[key] = {}
            protienmpnn_path = os.path.join(self.input_dir, 'protienmpnn', id_str)
            for path in os.listdir(protienmpnn_path):
                fasta_dir   = os.path.join(
                    protienmpnn_path, path, 'peptide_design', 'seqs'
                )
                fasta_files = [f for f in os.listdir(fasta_dir) if f.endswith('.fa')]
                if not fasta_files:
                    continue
                fasta_path = os.path.join(fasta_dir, fasta_files[0])
                for pep in extract_peptide_from_fasta(fasta_path):
                    # Track which ids a peptide was sampled from
                    PEPTIDE_POOLS[key].setdefault(pep, set()).add(id_str)
        FINAL_DF     = []
        FINAL_SCORES = {}
        for key in SCORES.keys():
            scores_stack   = np.stack(SCORES[key])                         # (K, 20, L)
            key_comb_score = (
                logsumexp(scores_stack, axis=0) - np.log(scores_stack.shape[0])
            )                                                               # (20, L)
            n_positions    = key_comb_score.shape[1]
            key_comb_score_df = pd.DataFrame(
                key_comb_score,
                columns=[str(i + 1) for i in range(n_positions)],
                index=list('ACDEFGHIKLMNPQRSTVWY')
            )
            pep_id_map   = PEPTIDE_POOLS[key]
            peptide_pool = list(pep_id_map.keys())
            self.logger.info(
                f"Scoring {len(peptide_pool)} unique {key}-mer peptides for {name}"
            )
            # Vectorised scoring – also divides by peptide length internally
            pep_scores   = _score_peptides_vec(peptide_pool, key_comb_score)
            sampled_from = [
                ';'.join(sorted(pep_id_map[pep])) for pep in peptide_pool
            ]
        ...
        self.logger.info(
            f"Saved peptide scores for {name}: {len(FINAL_DF)} peptides"
        )
        return FINAL_DF, FINAL_SCORES

    def combine_and_score_peptides_one_shot(self, df_with_pae_plddt, name):
        """
        One-shot / benchmarking mode.

        For each id under *name* independently:
          - Use only that id's combined score matrix.
          - Score only the peptides sampled from that id.
          - Emit one row per (peptide, id) with no merging across ids.
          - Divide score by peptide length (length normalisation).

        If the same peptide was sampled from two different ids, it appears
        in two separate rows, each carrying its own id and score.
        ...
        -------
        FINAL_DF : pd.DataFrame | None
            Columns: scores, peptides, kmer, name, id
        FINAL_SCORES : dict | None
            {id_str: {key: matrix_list, 'aa_index': list}}
        """
        subset_df = df_with_pae_plddt[df_with_pae_plddt['name'] == name].copy()
        if len(subset_df) == 0:
            self.logger.warning(f"[one_shot] No rows found for name={name}")
            return None, None
        output_path = os.path.join(self.output_dir, name)
        os.makedirs(output_path, exist_ok=True)
        if self.args.pae_threshold:
            subset_df = subset_df[subset_df['pae'] <= self.args.pae_threshold]
            self.logger.info(
                f"[one_shot][{name}] PAE threshold {self.args.pae_threshold}: "
                f"{len(subset_df)} rows remaining"
            )
        elif self.args.top_pae_percent:
            assert self.args.top_pae_percent <= 1.0
            ...
            subset_df = subset_df.iloc[:n_keep]
            self.logger.info(
                f"[one_shot][{name}] top {self.args.top_pae_percent * 100}% PAE: "
                f"{len(subset_df)} rows remaining"
            )
        if len(subset_df) == 0:
            raise ValueError(
                f"[one_shot] No valid PAE structures for name={name}."
            )
        FINAL_DF     = []
        FINAL_SCORES = {}
        for _, row in subset_df.iterrows():
            ...
            if not os.path.exists(comb_path):
                self.logger.warning(
                    f"[one_shot] combined_score.csv missing for {name}/{id_str}"
                )
                continue
            comb_score = pd.read_csv(comb_path, index_col=0).to_numpy()
            # Collect only the peptides sampled from this id
            protienmpnn_path = os.path.join(self.input_dir, 'protienmpnn', id_str)
            peptide_pool_id  = set()
            for path in os.listdir(protienmpnn_path):
                fasta_dir   = os.path.join(
                ...
                if not fasta_files:
                    continue
                fasta_path = os.path.join(fasta_dir, fasta_files[0])
                peptide_pool_id.update(extract_peptide_from_fasta(fasta_path))
            peptide_pool_id = list(peptide_pool_id)
            if not peptide_pool_id:
                self.logger.warning(
                    f"[one_shot] No peptides found for {name}/{id_str}"
                )
                continue
            self.logger.info(
                f"[one_shot][{name}/{id_str}] Scoring "
                f"{len(peptide_pool_id)} unique {key}-mer peptides"
            )
            # Vectorised scoring – divides by peptide length internally
            pep_scores = _score_peptides_vec(peptide_pool_id, comb_score)
            id_df = pd.DataFrame({
                'scores':   pep_scores,
                'peptides': peptide_pool_id,
                'kmer':     int(key),
                'name':     name,
                'id':       id_str,
            })
            FINAL_DF.append(id_df)
            FINAL_SCORES.setdefault(id_str, {})[key] = comb_score.tolist()
        if not FINAL_DF:
            return None, None
        FINAL_DF = pd.concat(FINAL_DF, ignore_index=True)
        for id_str in FINAL_SCORES:
            FINAL_SCORES[id_str]['aa_index'] = list('ACDEFGHIKLMNPQRSTVWY')
        FINAL_DF.to_csv(
            os.path.join(output_path, 'peptide_score_one_shot.csv'), index=False
        )
        with open(os.path.join(output_path, 'scores_one_shot.json'), 'w') as f:
            json.dump(FINAL_SCORES, f)
        self.logger.info(
            f"[one_shot][{name}] Saved {len(FINAL_DF)} rows "
            f"(one per peptide x id pair)"
        )
        return FINAL_DF, FINAL_SCORES
    ...
    def _visualize_score_matrix(self, score_df, name, kmer, output_path):
        ...
        sns.heatmap(
            score_df, cmap='viridis',
            annot=True if score_df.shape[1] <= 15 else False,
            fmt='.2f', ax=ax, cbar_kws={'label': 'Log Probability'}
        )
    ...
    def _visualize_pae_plddt_distributions(self, df):
        ...
        fig, axes = plt.subplots(
            2, max(n_names, 1),
            figsize=(max(5 * n_names, 6), 10), squeeze=False
        )
        for i, name in enumerate(names):
            subset = df[df['name'] == name]
            for j, col in enumerate(['pae', 'plddt']):
                ax = axes[j, i]
                sns.histplot(
                    subset[col].dropna(), kde=True if len(subset) > 2 else False,
                    ax=ax, color='steelblue' if col == 'pae' else 'salmon'
                )
                ax.set_xlabel(col.upper(), fontsize=10)
                ax.set_ylabel('Count', fontsize=10)
                ax.set_title(f'{name}\n{col.upper()} Distribution', fontsize=11)
        plt.tight_layout()
        save_path = os.path.join(self.output_dir, 'pae_plddt_distributions.png')
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close()
        self.logger.info(f"Saved PAE/pLDDT distributions: {save_path}")

    def _visualize_pae_vs_plddt_scatter(self, df):
        """Visualize PAE vs pLDDT scatter coloured by name."""
    ...
    def _visualize_peptide_score_distribution(self, all_results_df):
        """Visualize peptide score distributions per name."""
        ...
        fig, axes = plt.subplots(
            1, max(n_names, 1),
            figsize=(max(5 * n_names, 6), 5), squeeze=False
        )
        for i, name in enumerate(names):
            ...
            if len(subset) == 1:
                ...
                ax.set_xticks([0])
                ax.set_xticklabels(
                    [subset['peptides'].values[0]], rotation=45,
                    ha='right', fontsize=8
                )
            else:
                sns.histplot(
                    subset['scores'].dropna(),
                    kde=True if len(subset) > 2 else False,
                    ax=ax, color='coral'
                )
            ...
            if len(subset) > 0:
                ax.axvline(
                    subset['scores'].mean(), color='red', linestyle='--',
                    label=f'Mean: {subset["scores"].mean():.2f}'
                )
            ...
            for id_folder in os.listdir(name_dir):
                ...
                if os.path.isdir(id_path):
                    for fname in os.listdir(id_path):
                        ...
                        if (
                            '_mpnn_score.csv'     in fname or
                            '_hermes_score.csv'   in fname or
                            '_combined_score.csv' in fname
                        ):
                            os.remove(fpath)
    ...
    def _write_summary_log(self, df, all_results):
        ...
        if len(valid_pae) > 0:
            self.logger.info(
                f"PAE - Mean: {valid_pae.mean():.3f}, "
                f"Std: {valid_pae.std():.3f}, "
                f"Min: {valid_pae.min():.3f}, "
                f"Max: {valid_pae.max():.3f}"
            )
        if len(valid_plddt) > 0:
            self.logger.info(
                f"pLDDT - Mean: {valid_plddt.mean():.3f}, "
                f"Std: {valid_plddt.std():.3f}, "
                f"Min: {valid_plddt.min():.3f}, "
                f"Max: {valid_plddt.max():.3f}"
            )
        if all_results is not None and len(all_results) > 0:
            ...
            if len(valid_scores) > 0:
                self.logger.info(
                    f"Score - Mean: {valid_scores.mean():.3f}, "
    ...
    def run(self, df_path=None):
        ...
        Reads the input dataframe, computes PAE/pLDDT and score matrices for
        every (name, id) pair, then dispatches scoring to:
          - combine_and_score_peptides          (default)
          - combine_and_score_peptides_one_shot (--one_shot flag)

        Scores are divided by peptide length in both modes.
        """
        ...
        df['pae']   = PAE
        df['plddt'] = PLDDT
        pae_plddt_path = os.path.join(
            self.output_dir, 'pae_plddt_filtering.csv'
        )
        df[['pae', 'plddt', 'id', 'name']].to_csv(pae_plddt_path, index=False)
        self.logger.info(f"Saved PAE/pLDDT results to {pae_plddt_path}")
        if self.args.visualize:
        ...
        # Dispatch to the correct scoring method
        if self.args.one_shot:
            score_fn = self.combine_and_score_peptides_one_shot
            mode_tag = "one_shot"
        else:
            score_fn = self.combine_and_score_peptides
            mode_tag = "normal"
        self.logger.info(f"Running in [{mode_tag}] scoring mode")
        all_results = []
        for name in df['name'].unique():
            self.logger.info(f"[{mode_tag}] Combining scores for {name}...")
            try:
                final_df, _ = score_fn(df, name)
        ...
        if all_results:
            ...
            out_fname = (
                'all_peptide_scores_one_shot.csv'
                if self.args.one_shot
                else 'all_peptide_scores.csv'
            )
            combined_results.to_csv(
                os.path.join(self.output_dir, out_fname), index=False
            )
            self.logger.info(f"Saved combined results to {out_fname}")
            if self.args.visualize:
...
def parse_args():
    """Parse command-line arguments for the peptide scoring pipeline."""
    parser = argparse.ArgumentParser(
        description="Peptide scoring and evaluation pipeline"
    )
    parser.add_argument(
        "--input_dir",  type=str, required=True,
        help="Input directory containing protienmpnn and alphafold folders"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--df",         type=str, required=True,
        help="Path to input CSV/TSV file with peptide data"
    )
    parser.add_argument(
        "--radius",          type=float, default=10.0,
        help="Contact radius in Angstroms"
    )
    parser.add_argument(
        "--pae_threshold",   type=float, default=None,
        help="PAE threshold for filtering"
    )
    parser.add_argument(
        "--top_pae_percent", type=float, default=None,
        help="Top percentage of PAE to keep (0-1)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dirty_mode", action="store_true",
        help="Keep intermediate files in output directory"
    )
    parser.add_argument(
        "--force_run",  action="store_true",
        help="Continue running even if errors occur"
    )
    parser.add_argument(
        "--one_shot",   action="store_true",
        help=(
            "Benchmarking mode. Each peptide is scored only by the combined "
            "score of the id it was sampled from. If the same peptide appears "
            "in multiple ids it gets one row per id (no merging). "
            "Scores are divided by peptide length in all modes."
        )
    )
